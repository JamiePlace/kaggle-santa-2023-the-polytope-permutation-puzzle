...
class RandomGenerativeMoveSetGenerator(GenerativeMoveSetGenerator):
    ...
    def generate_moveset(self):
        LOGGER.info(f"--- Generating moveset using RandomGenerativeMoveSetGenerator ---")
    ...
    def change_random_move(self, allowed_moves, move, number):
        ...
        for x in range(number):
            changed_move[np.random.randint(0, len(changed_move))] = self.get_random_move_from_list_of_allowed_moves(
                allowed_moves)
