...
class IterativeGenerativeMoveSetGenerator(GenerativeMoveSetGenerator):
    """
    This is really only to be used for solving 1 specific puzzle.

    This generator will start from 1 move and depending if the errors/score improves or not we will either change up
    a move, or add a new move

    for example
    if the new score is better than before, then we need to add a move to get closer to the solution
    else
    the update move isnt any better, we scratch it and chnage the last move to something else in the hope for a
    better score
    and...
    we set some sort of upper limit to cull moves that grow too large, and add some sort of randomness to add moves
    back in for the ones we culled
    """
